...
def batch_embeddings(embeddings_generator, batch_size=None):
    ...
    If batch_size is None, yield the entire generator as a single batch.
    """
...
def compare_embeddings(large_embeddings_batches, query_embeddings):
    query_embeddings_torch = torch.from_numpy(query_embeddings).to(device="cuda")
    top_indices_accumulator = []
    top_cosine_similarities_accumulator = []
    ...
    for large_metadata_batch, large_embeddings_batch in large_embeddings_batches:
        ...
        top_indices = torch.topk(cosine_similarities, k=1, dim=1)[1]
        top_cosine_similarities = cosine_similarities.gather(1, top_indices)
    ...
    top_indices_into_accumulator = torch.topk(top_cosine_similarities_accumulator, k=1, dim=1)[1]
    top_indices_global = top_indices_accumulator.gather(1, top_indices_into_accumulator)
    top_cosine_similarities_global = top_cosine_similarities_accumulator.gather(1, top_indices_into_accumulator)
...
if __name__=="__main__":
    ...
    print("top_cosine_similarities.shape:", top_cosine_similarities.shape)
    print("top_indices:", top_indices)
    print("top_cosine_similarities:", top_cosine_similarities)
